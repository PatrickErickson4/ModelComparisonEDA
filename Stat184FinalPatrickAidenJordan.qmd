...
Based on the following information, we can perform the following operations to achieve a new, cleaned dataframe:
...
By looking at the head, we can see that this model is much easier to interpret.

### Initial Exploratory Data Analysis

In order to build up on the notion of prediction, We are particularly interested in the metric of wins and losses, as this is generally the metric of greatest interest for every basketball team, every season. Therefore, our data focus will be around **wins and losses**, and the potential relationships every variable has on this metric. We can view a simple bar graph to see if there is an imbalance between.
...
We can see here that there are clear differences in the averages of the teams that won and the averages of the teams that lost. Based on this difference, we can **infer** there might be some effect a higher or lower value of one of these variables can have, on the winrate of a game, respectively. We can see however, that based on the names, there might also be correlations between the variables. Let us model a scatterplot for the correlation between points and field
...
ggplot(chi_sq_data, aes(x = x, y = density)) +
  ...
  theme_minimal() +
  labs(
    title = "Chi-squared Distribution with 1 Degree of Freedom",
    x = "Chi-squared Value",
    y = "Density"
  ) +
  annotate(
...
ggplot(roc_data_combined, aes(x = 1 - specificity, y = sensitivity, color = Type)) +
  ...
  annotate(
    "text",
    x = 0.5, y = 0.2, 
    label = paste("Random AUC =", round(auc_random, 3)), 
    color = "red", 
    size = 5, 
    hjust = 0
  ) +
...
Here we can see the baseline AUC in blue, specifying that a model would not do any better than randomly guessing the outcome, and so we get a curve that mimics the probability of our initial ratio of wins to losses, 52%. Meanwhile, marked in green is the perfect case, where a model's efficacy is perfect. We are aiming to achieve the green with our model.
...
We can therefore infer that our model does improve our odds of predicting the binary classifier. **Keep in mind that this is not a measure of accuracy, rather it is a tool to be able to compare some models with others**

### Naively Fitting all of our predictors of interest (Make improvements to the models)

Understanding how we can compare two models' efficacies, we will now construct a new model, computing the following tests to see how well it does with models that we have already tested, and test the model for model significance. We will construct our null and alternatives as: 

$$H_0: \beta_1 = \beta_2 =\cdots = \beta_i = 0$$
$$H_1: \beta_1 = \beta_2 =\cdots = \beta_i \neq 0$$
essentially expanding our simple regression goodness of fit test for significance to include all of our parameters of interest. This is a total of 20 variables, which follows $\chi_{20}^2$.
...
For reference, this is what $X_{20}^2$ looks like without such outrageous boundaries:
...
In order to achieve a more parsimonious model, we can employ a method known as BIC backwards elimination, where we penalize poorer fitting predictors that are highly multicolinear by a large amount. BIC then picks the greatest calculated values per predictor to eliminate, effectively expelling them from our model. In general, BIC is a more rigorous elimination method than AIC, since BIC makes use the log of the number of predictors multiplied with some penalty, where AIC uses a constant to perform the same metric. Theoretically, we can obtain a simpler model that performs just as well, if not better due to reduced overfitting, by shaving off these predictors via this method.
...
ggplot(chi_sq_data, aes(x = x, y = density)) +
  ...
  theme_minimal() +
  labs(
    title = "Chi-squared Distribution with 14 Degrees of Freedom",
    x = "Chi-squared Value",
    y = "Density"
  ) +
  annotate(
    ...
    y = 0.0005, yend = 0.00013,    # Starting and ending y-coordinates for the arrow
    arrow = arrow(length = unit(0.2, "cm"), type = "closed"),  # Arrow settings
    color = "red",       # Arrow color
    size = 1             # Arrow thickness
  ) +
...
We can see the difference between the AUC for the BIC model and our original model is almost negligible, with a .0001 difference between the two.
...
max_nodes <- max(input_nodes, hidden_nodes, output_nodes)
input_y <- seq(1, max_nodes, length.out = input_nodes)
hidden_y <- seq(1 + (max_nodes - hidden_nodes) / 2, hidden_nodes + (max_nodes - hidden_nodes) / 2, length.out = hidden_nodes)
output_y <- seq(1 + (max_nodes - output_nodes) / 2, output_nodes + (max_nodes - output_nodes) / 2, length.out = output_nodes)
...
ggplot() +
  ...
  geom_segment(data = connections, aes(x = x_from, y = y_from, xend = x_to, yend = y_to), alpha = 0.3) +
  # Add labels for layers
  annotate("text", x = 1.15, y = max_nodes + 2, label = "Input Layer\n(20 nodes)", size = 5, hjust = 0.5) +
  annotate("text", x = 2, y = max_nodes + 2, label = "Hidden Layer\n(ReLU)", size = 5, hjust = 0.5) +
  annotate("text", x = 2.85, y = max_nodes + 2, label = "Output Layer\n(Sigmoid)", size = 5, hjust = 0.5) +
  # Customize the plot
  theme_void() +
...
##NN model with tuned hyperparameters after sweeps based on intuition, and comparing 
...
-   Obtain more computing to be able to represent all of our categorcial variables in 1-hot and implement k-fold cross-validation
